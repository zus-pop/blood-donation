      ...
      if (attendedParts.length > 0) {
        ...
        if (eventToRegister) {
          ...
          if (eventStart <= latestAttendedEnd) {
            toast.error(
              "Bạn không thể đăng ký sự kiện có thời gian trước hoặc trùng với sự kiện bạn đã hiến máu gần nhất."
            );
            return;
          }
          if (eventStart <= threeMonthsAfter) {
            toast.error(
              "Bạn đã tham gia 1 sự kiện hiến máu trong vòng 3 tháng trở lại đây, vui lòng chờ đủ 3 tháng để đăng ký sự kiện mới."
            );
            return;
          }
        }
          ...
          <div className="space-y-4">
            {renderEvents.map((event) => {
              ...
              return (
                <Card
                  key={event._id}
                  className="overflow-hidden rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 group bg-white flex flex-col md:flex-row items-stretch"
                >
