...
export async function findBloodRequestById(id: string) {
    const bloodRequest = await BloodRequest.findById(id).populate("userId");
    if (!bloodRequest) {
...
export async function updateBloodRequest(id: string, data: Partial<BloodRequestInput>) {
    const bloodRequest = await BloodRequest.findByIdAndUpdate(id, data, { new: true }).populate("userId");
    if (!bloodRequest) {
        throw new Error("Blood request not found");
    }
    return bloodRequest;
